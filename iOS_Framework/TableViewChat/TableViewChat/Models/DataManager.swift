...
class DataManager {
    ...
    func makeMovieData() {
        ...
        movieDataArray = [
            ...
            Movie(movieImage: UIImage(named: "guardians.png"), movieName: "가디언즈 오브 갤럭시", movieDescription: "빌런 타노스에 맞서서 세상을 지키려는 가오갤 맴버들")
        ]
    }
    ...
    func updateMovieData() {
        ...
        print("updateMovieData의 프린트 : \(movieDataArray)")
    }
    
    func addMovieData(name: String, description: String) {
        print(#function)
        let image = UIImage(named: "spiderman2.png")
        let movie = Movie(movieImage: image, movieName: name, movieDescription: description)
        movieDataArray.append(movie)
        print("addMovieData의 프린트 : \(movieDataArray)")
